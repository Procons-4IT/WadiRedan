...
#Region "Update Pallet"
    ...
    Public Sub CalculateDiscount(ByVal aform As SAPbouiCOM.Form, ByVal aRow As Integer, ByVal aCol As String)
        Try
            aform.Freeze(True)
            Dim strItemCode, strSQL, strCardCode As String
            Dim dblNoofPallet, dblNoofEach, dblNoLay, dblQty, dblLineQty, dblLinePrice, dblLineUnitPrice, dblDiscount, dblDiscountPercentage, dblPriceafterDiscount As Double
            Dim oRec, oRec1, oTest As SAPbobsCOM.Recordset
            oMatrix = aform.Items.Item("38").Specific
            oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRec1 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            ...
            If strItemCode <> "" Then
                oRec1.DoQuery("Select isnull(T0.Price,0) 'Price' from ITM1 T0 inner Join OCRD T1 on T1.ListNum=T0.PriceList and T1.CardCode='" & strCardCode & "' and T0.ItemCode='" & strItemCode & "'")
                dblLinePrice = oRec1.Fields.Item("Price").Value
                If aCol = "U_Z_DISCOUNT" Then
                    dblLineUnitPrice = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "14", aRow))
                    dblLineQty = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "11", aRow))
                    ...
                    dblDiscount = getDocumentQuantity(getMatrixValues(oMatrix, "U_Z_DISCOUNT", aRow))
                    dblPriceafterDiscount = (dblNoofPallet * 100) / dblLineUnitPrice '  dblLineUnitPrice * dblNoofPallet / 100
                    SetMatrixValues(oMatrix, "15", aRow, dblPriceafterDiscount)
                Else
                    dblLineUnitPrice = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "14", aRow))
                    dblLineQty = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "11", aRow))
                    ...
                    End Try
                    dblPriceafterDiscount = (dblQty * 100) / dblLineUnitPrice ' dblLineUnitPrice * dblQty / 100
                    SetMatrixValues(oMatrix, "15", aRow, dblPriceafterDiscount)
                End If
