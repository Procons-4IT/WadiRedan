...
#End Region
    Public Sub SetEditText(ByVal aForm As SAPbouiCOM.Form, ByVal aUID As String, ByVal aVal As String)
        ...
        objedit.Value = aVal
    End Sub
#Region "Create GoodsIssue"
    Public Function creategoodsIssue(ByVal aDocEntry As Integer) As Boolean
        ...
        If oDoc.GetByKey(aDocEntry) Then
            For intRow As Integer = 0 To oDoc.Lines.Count - 1
                ...
                If oDoc.Lines.BaseType = 17 And oDoc.Lines.UserFields.Fields.Item("U_Z_AddRef").Value <> "" Then
                    oGoodsIssue = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oInventoryGenExit)
                    Dim blnLineexist As Boolean = False
                    oRec.DoQuery("Select * from [@Z_ADC2] where U_Z_Quantity>0 and U_Z_AddRef='" & oDoc.Lines.UserFields.Fields.Item("U_Z_AddRef").Value & "'")
                    For intLoop As Integer = 0 To oRec.RecordCount - 1
                        oGoodsIssue.DocDate = oDoc.DocDate
                        If intLoop > 0 Then
                            oGoodsIssue.Lines.Add()
                        End If
                        oGoodsIssue.Lines.SetCurrentLine(intLoop)
                        oGoodsIssue.Lines.ItemCode = oRec.Fields.Item("U_Z_ItemCode").Value
                        oGoodsIssue.Lines.Quantity = oRec.Fields.Item("U_Z_Quantity").Value
                        oGoodsIssue.Lines.Price = oRec.Fields.Item("U_Z_Price").Value
                        oGoodsIssue.Comments = "Created based on Additional Reference : " & oDoc.Lines.UserFields.Fields.Item("U_Z_AddRef").Value
        ...
        Return True
    End Function
    Public Function UpdateBOM(ByVal aMatrix As SAPbouiCOM.Matrix) As Boolean
        Dim oDoc As SAPbobsCOM.ProductTrees
        Dim oRec As SAPbobsCOM.Recordset
        oDoc = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oProductTrees)
        oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim aItemCode, aRefCode As String
        For intL As Integer = 1 To aMatrix.RowCount
            aItemCode = getMatrixValues(aMatrix, "1", intL)
            aRefCode = getMatrixValues(aMatrix, "U_Z_AddRef", intL)
            If aRefCode <> "" Then
                If oDoc.GetByKey(aItemCode) Then
                    If oDoc.TreeType = SAPbobsCOM.BoItemTreeTypes.iAssemblyTree Then
                        For intRow As Integer = oDoc.Items.Count - 1 To 0 Step -1
                            oDoc.Items.SetCurrentLine(intRow)
                            If oDoc.Items.UserFields.Fields.Item("U_Z_Type").Value = "A" Then
                                oDoc.Items.Delete()
                            End If
                        Next
                        ...
                        Else
                            If oDoc.GetByKey(aItemCode) Then
                                Dim blnLineexist As Boolean = False
                                Dim intLineCount As Integer = oDoc.Items.Count - 1
                                intLineCount = intLineCount + 1
                                oRec.DoQuery("Select * from [@Z_ADC2] where U_Z_Quantity > 0 and U_Z_AddRef='" & aRefCode & "'")
                                For intLoop As Integer = 0 To oRec.RecordCount - 1
                                    If intLineCount > 0 Then
                                        oDoc.Items.Add()
                                    End If
                                    oDoc.Items.SetCurrentLine(intLineCount)
                                    oDoc.Items.ItemCode = oRec.Fields.Item("U_Z_ItemCode").Value
                                    oDoc.Items.Quantity = oRec.Fields.Item("U_Z_Quantity").Value
                                    '      oDoc.Items.Price = oRec.Fields.Item("U_Z_Price").Value
                                    oDoc.Items.UserFields.Fields.Item("U_Z_Type").Value = "A"
                                    intLineCount = intLineCount + 1
                                    blnLineexist = True
                                    oRec.MoveNext()
                                Next
    ...
    Public Function RemoveBoMItem(ByVal aDocEntry As Integer) As Boolean
        Dim oDoc As SAPbobsCOM.ProductTrees
        Dim oDoc1 As SAPbobsCOM.Documents
        Dim oRec As SAPbobsCOM.Recordset
        oDoc = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oProductTrees)
        oDoc1 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oDeliveryNotes)
        oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim aItemCode, aRefCode As String
        If oDoc1.GetByKey(aDocEntry) Then
            For intL As Integer = 0 To oDoc1.Lines.Count - 1
                oDoc1.Lines.SetCurrentLine(intL)
                aItemCode = oDoc1.Lines.ItemCode
                aRefCode = oDoc1.Lines.UserFields.Fields.Item("U_Z_AddRef").Value
                If aRefCode <> "" Then
                    If oDoc.GetByKey(aItemCode) Then
                        If oDoc.TreeType = SAPbobsCOM.BoItemTreeTypes.iAssemblyTree Then
                            For intRow As Integer = oDoc.Items.Count - 1 To 0 Step -1
...
#Region "Update Pallet"
    Public Sub CalculatePallet(ByVal aform As SAPbouiCOM.Form, ByVal aRow As Integer)
        Try
            aform.Freeze(True)
            Dim strItemCode, strSQL As String
            Dim dblNoofPallet, dblNoofEach, dblNoLay, dblQty, dblLineQty, dblItemPallet, dblItemLayer As Double
            Dim oRec, oRec1, oTest As SAPbobsCOM.Recordset
            oMatrix = aform.Items.Item("38").Specific
            oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRec1 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            strItemCode = getMatrixValues(oMatrix, "1", aRow)
            If strItemCode <> "" Then
                dblLineQty = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "11", aRow))
                oRec.DoQuery("Select * from OITM where ItemCode='" & strItemCode & "'")
                If oRec.RecordCount > 0 Then
                    dblItemPallet = oRec.Fields.Item("U_Z_Pallet").Value
                    dblItemLayer = oRec.Fields.Item("U_Z_layer").Value

                End If
                If dblLineQty > 0 Then
                    If dblItemPallet > 0 Then
                        strSQL = "SELECT Floor(" & dblLineQty & "/T0.[U_Z_Pallet]) FROM OITM T0 WHERE T0.[ItemCode]='" & strItemCode & "'"
                        oRec.DoQuery(strSQL)
                        dblNoofPallet = oRec.Fields.Item(0).Value
                    Else
                        dblNoofPallet = 0
                    End If
                    SetMatrixValues(oMatrix, "U_Z_PALLET", aRow, dblNoofPallet)
                    If dblItemLayer > 0 Then
                        strSQL = "SELECT isnull(Floor(( " & dblLineQty & " - Floor(" & dblLineQty & " /T0.[U_Z_Pallet])*T0.[U_Z_Pallet])/T0.[U_Z_layer]),0)  FROM OITM T0 WHERE T0.[ItemCode]='" & strItemCode & "'"

                        oRec.DoQuery(strSQL)
                        dblNoLay = oRec.Fields.Item(0).Value
                    Else
                        dblNoLay = 0
                    End If
                    SetMatrixValues(oMatrix, "U_Z_LAYER", aRow, dblNoLay)
                    strSQL = "SELECT " & dblLineQty & "  - (Floor(" & dblLineQty & " /T0.[U_Z_Pallet])* T0.[U_Z_Pallet]+(Floor((" & dblLineQty & "  - Floor(" & dblLineQty & " /T0.[U_Z_Pallet])*T0.[U_Z_Pallet])/case T0.[U_Z_layer] when 0 then 1 else T0.U_Z_Layer end))*T0.[U_Z_Layer])  FROM OITM T0 WHERE T0.[ItemCode]='" & strItemCode & "'"
                    oRec.DoQuery(strSQL)
                    dblNoofEach = oRec.Fields.Item(0).Value
                    SetMatrixValues(oMatrix, "U_Z_ECH", aRow, dblNoofEach)
                End If
    ...
    Public Sub CalculateQTY(ByVal aform As SAPbouiCOM.Form, ByVal aRow As Integer)
        Try
            aform.Freeze(True)
            Dim strItemCode, strSQL As String
            Dim dblNoofPallet, dblNoofEach, dblNoLay, dblQty, dblLineQty As Double
            Dim oRec, oRec1, oTest As SAPbobsCOM.Recordset
            oMatrix = aform.Items.Item("38").Specific
            oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oRec1 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            ...
            If strItemCode <> "" Then
                dblLineQty = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "11", aRow))
                dblNoofPallet = getDocumentQuantity(getMatrixValues(oMatrix, "U_Z_PALLET", aRow))
                dblNoLay = getDocumentQuantity(getMatrixValues(oMatrix, "U_Z_LAYER", aRow))
                dblNoofEach = getDocumentQuantity(getMatrixValues(oMatrix, "U_Z_ECH", aRow))
                strSQL = "Select isnull(U_Z_Pallet,0),isnull(U_Z_Layer,0) from OITM where itemcode='" & strItemCode & "'"
                oRec.DoQuery(strSQL)
                dblLineQty = (dblNoofPallet * oRec.Fields.Item(0).Value) + dblNoLay * oRec.Fields.Item(1).Value + dblNoofEach
                If dblLineQty > 0 Then
                    SetMatrixValues(oMatrix, "11", aRow, dblLineQty)
                End If
            End If
            aform.Freeze(False)
        Catch ex As Exception
    ...
    Public Sub CalculateDiscount(ByVal aform As SAPbouiCOM.Form, ByVal aRow As Integer, ByVal aCol As String)
        Try
            aform.Freeze(True)
            Dim strItemCode, strSQL As String
            Dim dblNoofPallet, dblNoofEach, dblNoLay, dblQty, dblLineQty As Double
            ...
            strItemCode = getMatrixValues(oMatrix, "1", aRow)
            If strItemCode <> "" Then
                If aCol = "U_Z_DISCOUNT" Then
                    dblLineQty = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "11", aRow))
                    dblNoofPallet = getDocumentQuantity(getMatrixValues(oMatrix, "U_Z_DISCOUNT", aRow))
                    dblNoLay = getDocumentQuantity(getMatrixValues(oMatrix, "14", aRow))
                    dblQty = dblLineQty * dblNoofPallet
                    dblNoofEach = dblNoLay * dblLineQty
                    SetMatrixValues(oMatrix, "U_TotalDis", aRow, dblQty)
                    SetMatrixValues(oMatrix, "U_TotalBefDis", aRow, dblNoofEach)
                    SetMatrixValues(oMatrix, "U_Z_DISCOUNT", aRow, dblQty / dblLineQty)
                Else
                    dblLineQty = getDocumentQuantity(oApplication.Utilities.getMatrixValues(oMatrix, "11", aRow))
                    dblNoofPallet = getDocumentQuantity(getMatrixValues(oMatrix, "U_TotalDis", aRow))
                    dblNoLay = getDocumentQuantity(getMatrixValues(oMatrix, "14", aRow))
                    dblQty = dblNoofPallet / dblLineQty
                    dblNoofEach = dblNoLay * dblLineQty
                    SetMatrixValues(oMatrix, "U_TotalBefDis", aRow, dblNoofEach)
                    Try
                        SetMatrixValues(oMatrix, "U_Z_DISCOUNT", aRow, dblQty)
                    Catch ex As Exception
                        SetMatrixValues(oMatrix, "U_Z_DISCOUNT", aRow, Math.Round(dblQty, 0))
                    End Try
                End If
            End If
            aform.Freeze(False)
